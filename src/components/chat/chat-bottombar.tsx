      ...
      <form onSubmit={handleSubmit} className="relative w-full md:px-4">
        ...
        <div className="mx-auto flex items-center rounded-full border border-black/10 bg-white/5 py-2 pr-2 pl-6 backdrop-blur-lg shadow-[inset_0_1px_0_0_rgba(255,255,255,0.1)] transition-colors hover:bg-black/10 dark:border-white/20 dark:bg-black/10 dark:hover:bg-black/20">
          <input
            ...
            className="text-md w-full border-none bg-transparent text-black placeholder:text-gray-500 focus:outline-none dark:text-white dark:placeholder:text-gray-400"
          ...
          <button
            ...
            className="flex items-center justify-center rounded-full bg-[#0171E3] p-2 text-white transition-colors hover:bg-blue-600 disabled:opacity-50"
            onClick={(e) => {
              if (isLoading) {
                e.preventDefault();
                stop();
