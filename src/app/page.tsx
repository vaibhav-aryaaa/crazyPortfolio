...
// import GitHubButton from 'react-github-btn'; // This wasn't used in your JSX, but uncomment if needed

/* ---------- NEW Page Navigation Config ---------- */
const questionConfig = [
  { key: 'Me', color: '#329696', icon: Laugh, path: '/me' },
  { key: 'Projects', color: '#3E9858', icon: BriefcaseBusiness, path: '/projects' },
  { key: 'Skills', color: '#856ED9', icon: Layers, path: '/skills' },
  { key: 'Fun', color: '#B95F9D', icon: PartyPopper, path: '/fun' },
  { key: 'Contact', color: '#C19433', icon: UserRoundSearch, path: '/contact' },
] as const;

/* ---------- component ---------- */
export default function Home() {
  // const [input, setInput] = useState(''); // No longer needed
  // const inputRef = useRef<HTMLInputElement>(null); // No longer needed
  ...
  useEffect(() => {
    ...
    linkWebm.as = 'video';
    linkWebm.href = '/vaibhav-memoji.webm'; // Corrected from '/final_memojis.webm'
    document.head.appendChild(linkWebm);

    const linkMp4 = document.createElement('link');
    linkMp4.rel = 'prefetch';
    linkMp4.as = 'video';
    linkMp4.href = '/vaibhav-memoji.mp4'; // Corrected from '/final_memojis_ios.mp4'
  ...
  return (
    <div className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden px-4 pb-10 md:pb-20">
      ...
      >
        {/* free-form question (REMOVED) */}

        {/* quick-question grid */}
        <div className="mt-4 grid w-full max-w-2xl grid-cols-1 gap-3 sm:grid-cols-3 md:grid-cols-5">
          {questionConfig.map(({ key, color, icon: Icon, path }) => (
            <Button
              key={key}
              onClick={() => navigateToSection(path)} // <-- UPDATED
              variant="outline"
